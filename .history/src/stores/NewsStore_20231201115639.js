...
class NewsStore {
    ...
    getTop5CategoryHeadlinesList = async (updateCategory) => {
        CATEGORIES.forEach(async (category) => {
            const { data, error } = await NewsService.getTop5CategoryHeadlinesList(category, this.activeCountry);
            if (data) {
    ...
    setActiveCountry = (country) => {
        ...
        this.getTop5CategoryHeadlinesList()
